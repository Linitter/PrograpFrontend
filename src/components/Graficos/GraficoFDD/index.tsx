...
export default function GraficoFDD() {
  ...
  const [FDD, setFDD] = useState<any[]>([]); // obejtos/recusos
  ...
  useEffect(() => {
    if (objectResource.length > 0) {
      ...
      const convenantsObjects = objectResource.filter(
        obj => obj.fdd !== undefined && obj.fdd !== null,
      );
  ...
  const findFdd = FDD.filter(
    (item: any) => item.globalValue && item.deleted_at === null,
  );

  findFdd.forEach((item: any) => {
  ...
  const donutChartData = Object.values(fddInfo).map((info: any) => {
  ...
  const legendLabels = Object.keys(fddInfo).map(agreementNumberFdd => {
    const { globalValue } = fddInfo[agreementNumberFdd];
